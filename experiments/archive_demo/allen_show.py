...
slab image; the decoded structure layer drawn on it (one colour per separate
...
def main():
    ...
    fig, ax = plt.subplots(len(picks), 3, figsize=(10.5, 3.4 * len(picks)))
    ...
    for i, r in enumerate(picks):
        ...
        im, cell, _, _ = slab_projection(raw, seg, ast.literal_eval(m.name_dict),
            ...
            if j > 0:
                for u, v in edges:
                    ...
                    A.plot([pos[u][1], pos[v][1]], [pos[u][0], pos[v][0]], color=c,
                           lw=1.3 if j == 1 else 1.8, solid_capstyle='round')
                kinds = st['vkind']
        ...
        ax[i, 2].text(0.02, 0.98,
                      f'{len(sb)} bytes stored\n'
                      f'{int(r.n_components)} mitochondria, {int(r.n_junctions)} junctions\n'
                      f'{r.total_length_um:.0f} µm total length\n'
                      f'{r.components_per_100um:.1f} separate / 100 µm\n'
                      f'mean branch {r.mean_branch_um:.2f} µm',
                      transform=ax[i, 2].transAxes, va='top', ha='left', color='white', fontsize=7.5,
                      family='monospace')
    for j, t in enumerate(['mitochondria (TOMM20), ±1 µm slab; 5 µm bar',
                           'stored structure layer, drawn on the image',
                           'structure layer alone + what it answers']):
