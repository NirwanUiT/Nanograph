...
#
# Resumable copy of run_paper_v3.sh (identical steps):
#   FROM=<step>   skip every step before <step> (a phase-1 outdir, or one of
#                 downstream, ablation_bg_residual, seg_perturbation_ablation,
#                 heldout, sim_truth, clip_truth, real_downstream, real_segmenters)
#   ONLY=<step>   run just that step
set -uo pipefail
...
ONLY=${ONLY:-}
mkdir -p "$OUT"
[ -n "$FROM$ONLY" ] || echo "$HASH" > "$OUT/commit.txt"

stamp () { echo "$HASH" > "$1/commit.txt"; }
log () { echo "=== [$(date +%H:%M:%S)] $*"; }
# go <step>: 0 if the step should run under FROM/ONLY
go () {
  if [ -n "$ONLY" ]; then [ "$1" = "$ONLY" ]; return; fi
  if [ -n "$FROM" ]; then
    if [ "$1" = "$FROM" ]; then FROM=; else log "skip $1 (resume)"; return 1; fi
  fi
}
rd () { local out=$1; shift; go "$out" || return 0; log "$out"; python run_dataset.py "$@" --outdir "$OUT/$out" && stamp "$OUT/$out"; }
