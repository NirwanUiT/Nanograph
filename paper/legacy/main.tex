...
foreground at 28.4\,dB FG-PSNR / 0.997 FG-SSIM; at equal bytes it reproduces
its own structural mask more faithfully than JPEG on 77\% of images (self-IoU)
and matches JPEG on ground-truth foreground recovery. The representation
transfers across five microscopy modalities. We further analyse the pipeline's dominant bottleneck,
...
(3) We show the representation is a viable \emph{compressor}: on 726 organelle
images it uses $\approx$\,2.5\,kB/image and is statistically at parity with
byte-matched JPEG on ground-truth foreground recovery (GT-IoU wins 49\%, sign
test $p=0.53$), while---unlike a codec---exposing morphometry and topology for
...
foreground-restricted FG-PSNR/FG-SSIM, \emph{self-IoU} (thresholded
reconstruction vs.\ the pipeline's \emph{own} foreground mask, i.e.\ how
faithfully the codec reproduces the structure it chose to encode),
\emph{GT-IoU} (thresholded reconstruction vs.\ ground-truth foreground),
payload size in bytes, and graph statistics. As a compression baseline we
encode each image with JPEG at the quality that matches \method's byte budget,
and compare fidelity at equal bytes.

\begin{table}[t]
\centering
\caption{\method on 726 organelle images (mean$\pm$std). Right: fraction of
images where \method beats \emph{byte-matched} JPEG; two-sided sign tests give
$p<10^{-48}$ for self-IoU and $p=0.53$ (parity) for GT-IoU.}
\label{tab:nmi}
...
Self-IoU             & $0.433\pm0.089$& \multicolumn{2}{l}{}\\
...
budget} it reproduces its own structural mask more faithfully than JPEG on 77\%
of images (self-IoU; sign test $p<10^{-48}$, 95\% CI $[74\%,80\%]$), because it
spends its bits on structure rather than background texture. Measured against
\emph{ground-truth} foreground, however, the two codecs are statistically
indistinguishable (GT-IoU wins $354/726$, $p=0.53$): \method's advantage is
structure-targeted bit allocation, not higher absolute fidelity to the
annotation. Unlike a codec, the same payload directly yields
...
self-IoU for 82--100\% of images across datasets (two-sided sign test
$p<10^{-3}$ for all but the five-image Cells set, $p=0.06$), suggesting the
graph representation is not specific to organelles. These four datasets carry
no ground-truth foreground annotations, so the GT-backed codec comparison that
ends at parity on organelles (Sec.~\ref{sec:repr}) cannot be run here; the
reported wins measure structural self-consistency only.

\begin{table}[t]
\centering
\caption{Same encoder across five modalities (mean$\pm$std). Self-IoU wins vs.\
byte-matched JPEG; sign test $p<10^{-3}$ everywhere except Cells ($n{=}5$,
$p=0.06$, not significant). Only organelles have ground-truth masks
(Table~\ref{tab:nmi}).}
\label{tab:cross}
...
$N$ & 726 & 60 & 60 & 25 & 5\\
\midrule
FG-SSIM       & $.997{\pm}.001$ & $.982{\pm}.012$ & $.966{\pm}.013$ & $.985{\pm}.006$ & $.984{\pm}.006$\\
FG-PSNR (dB)  & $28.4{\pm}0.4$  & $31.2{\pm}1.9$  & $27.2{\pm}3.0$  & $33.1{\pm}0.6$ & $34.5{\pm}0.7$\\
Self-IoU wins & 77\%  & 82\%  & 83\%  & 88\%  & 100\%\\
\bottomrule
\end{tabular}
\end{table}
...
reconstruction fidelity (self- and GT-IoU alike) is bounded by segmentation.
Against ground truth, compression is at parity with byte-matched JPEG rather
than ahead of it; the case for the representation rests on the analysis it
exposes, not on rate--distortion alone. The external datasets are small
...
compresses at parity with JPEG at equal bytes while exposing morphometry and
